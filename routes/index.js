		...
		labyokerCats.getcategories(function(error, categories) {
			...
			labYokerGetOrder.getorders(function(error, results) {
				...
				if(results != null){
					...
					booster.push("<strong> Self Information!</strong> You have ordered a total of <b>" + totalorders + " order(s)</b> and received a total of <b>" + totalshares + " requested share(s)</b>. Keep it up!");
		...
		if (req.session.user) {
			...
			labYokerorder.order(function(error, results) {
				if(results != null && results=="successfulOrder"){
					...
					res.render('orders', {booster:req.session.savingsText, boostercolor:req.session.savingsColor, lab:req.session.lab, categories: req.session.categories, ordersnum: req.session.orders, sharesnum: req.session.shares, labyoker : req.session.user, isLoggedInAdmin: req.session.admin, title:'Orders',loggedIn : true, location: location, agent: agent, vendor: vendor, catalog: catalognumber, email: email});
					req.session.messages = null;
				}
			});
		} else {
