		...
		if (req.session.user) {
			...
			labYokeSearch.search(function(error, results) {
				...
				if (searchText != null && searchText.length > 0){
					...
					res.render('search', {message:'You entered an invalid reagent search keyword. Please try again.',mylab: req.session.lab,ordersnum: req.session.orders, sharesnum: req.session.shares, labyoker : req.session.user, isLoggedInAdmin: req.session.admin, title: 'Search', loggedIn : true, agentsResults : results[1]});
				}
				req.session.messages = null;
			});
		} else {
		...
		if (req.session.user) {
			...
			labYokeSearch.search(function(error, results) {
				...
				if (searchText != null && searchText.length > 0){
					...
					res.render('search', {messageCatalog:'You entered an invalid catalog search keyword. Please try again.',mylab: req.session.lab,ordersnum: req.session.orders, sharesnum: req.session.shares, labyoker : req.session.user, isLoggedInAdmin: req.session.admin, title: 'Search', loggedIn : true, agentsResults : results[1]});
				}
				req.session.messages = null;
			});
		} else {
