...
bot.dialog('/order', [
  ...
  function (session, args) {
    console.log("ordering? " + session.userData);
    console.log("ordering? " + session.userData.agent);
    console.log("ordering session.dialogData? " + args[0]);
  }
])
...
*/
    ...
    router.get('/admins', isLoggedInAdmin, function(req, res) {
        ...
        labYokeAgents.findallsharesadmins(function(error, results) {
            ...
            res.render('admins', {meadminof:req.session.meadminof,lang:req.cookies.i18n, i18n:res, currentlabname: req.session.meadminof, ordersnum: req.session.orders, sharesnum: req.session.shares, labyoker : req.session.user, labyokersurname : req.session.surname, myshares: results[1], mysharesrequest: results[4], report_data: results[2], loggedIn : true, isLoggedInAdmin: req.session.admin, title:'Admins'});
            req.session.messages = null;
        });
    } else {
        res.redirect("/login");
